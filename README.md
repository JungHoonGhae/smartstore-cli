...
네이버는 [공식 커머스 API](https://apicenter.commerce.naver.com)를 제공하지만, 판매자센터 웹에서만 볼 수 있는 데이터는 공식 API에 포함되지 않거나 유료 구독이 필요합니다.

| 데이터 | 공식 커머스 API | storectl |
|--------|---------------|----------|
| 상품 등록/수정/조회 | **제공** (21개 API) | 조회만 |
| 주문 조회/발송처리 | **제공** (5개 API) | 조회만 |
| 정산 | **제공** (2개 API) | 대시보드 |
| 문의 | **제공** (3개 API) | 대시보드 |
| 판매자 등급/페널티 | **미제공** | **제공** |
| 리뷰 | **미제공** | **제공** |
...
storectl은 이 **판매자센터 전용 데이터**를 CLI로 가져옵니다.
