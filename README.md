...
네이버는 [공식 커머스 API](https://apicenter.commerce.naver.com)를 제공하지만, 사용하려면 개발사 계정 가입 → 애플리케이션 등록(심사) → OAuth 2.0 인증 구현이 필요합니다. **일반 판매자에게는 사실상 접근 불가능한 진입 장벽입니다.**
...
|--------|---------------|----------|
| 상품 조회 | 제공 (OAuth 필요) | **로그인만으로 조회** |
| 주문 조회 | 제공 (OAuth 필요) | **로그인만으로 조회** |
| 판매자 등급/페널티 | **미제공** | **제공** |
| 리뷰 | **미제공** | **제공** |
| 알림 | **미제공** | **제공** |
...
storectl은 **OAuth 셋업 없이 브라우저 로그인만으로** 판매자센터 데이터를 CLI에서 조회합니다.
...
### v0.1.0 — Read-Only Foundation (현재)

- [x] 브라우저 기반 로그인 (Playwright + Google Chrome)
- [x] 실제 API 리버스 엔지니어링 (513개 엔드포인트 발견)
- [x] 판매자 정보/등급 조회
- [x] 상품 목록/상세/대시보드
- [x] 주문/배송 대시보드 + GraphQL 주문 목록
- [x] 매출 통계 (일간/주간/월간)
- [x] 정산/문의/리뷰/알림 조회
- [x] 페이지네이션 (`--page`, `--size`)
- [x] 세션 자동 갱신 + 초기화 자동화
- [x] Table/JSON/CSV 출력
- [x] 5개 플랫폼 릴리즈 (macOS/Linux/Windows)
- [x] 25개 유닛 테스트

### v0.2.0 — Write Operations (예정)

- [ ] 상품 등록 (`storectl product create`)
- [ ] 상품 수정 (`storectl product update`)
- [ ] Safety gate: `config.json`으로 쓰기 기능 기본 비활성화
- [ ] 상품 등록 API 리버스 엔지니어링

### v0.3.0 — Stability & DX (예정)

- [ ] Homebrew tap (`brew install storectl`)
- [ ] 세션 자동 재로그인 안정화
...
- [ ] 주문 상태 변경 (발송처리, 취소 등)
- [ ] 일괄 상품 등록 (`storectl product bulk-create`)
- [ ] 알림 실시간 모니터링
